...
scatter_plot(p, p$bill_length_mm, p$flipper_length_mm, p$species, 10, 50, c("bill length", "in mm"), c("flipper length", "in mm"), "Legend") %>% 
  add_title("Relationship between bill length and flipper length","","") %>% SVGrenderer(height=300)
#---
p <- penguins %>%
...
scatter_plot(p, p$bill_length_mm, p$body_mass_g, p$species, 10, 1000, c("bill length", "in mm"), c("body mass", "in g"), "Legend") %>% 
  add_title("Relationship between bill length and body mass","","") %>% SVGrenderer(height=300)
#--- bill depht on the x -axis ---
p <- penguins %>%
  drop_na(bill_depth_mm, bill_length_mm)
scatter_plot(p, p$bill_depth_mm, p$bill_length_mm, p$species, 5, 10, c("bill depht", "in mm"), c("bill length", "in mm"), "Legend") %>% add_title("Relationship between bill depth and bill length","","") %>% SVGrenderer(height=300)
#----
p <- penguins %>%
  drop_na(bill_depth_mm, flipper_length_mm)
scatter_plot(p, p$bill_depth_mm, p$flipper_length_mm, p$species, 5, 50, c("bill depht", "in mm"), c("flipper length", "in mm"), "Legend") %>% add_title("Relationship between bill depth and flipper length","","") %>%  SVGrenderer(height=300)
#----
p <- penguins %>%
  drop_na(bill_depth_mm, body_mass_g)
scatter_plot(p, p$bill_depth_mm, p$body_mass_g, p$species, 5, 1000, c("bill depht", "in mm"), c("body mass", "in g"), "Legend") %>% add_title("Relationship between bill length and body mass","","") %>% SVGrenderer(height=300)
#---flipper length on the x axis ----
p <- penguins %>%
  drop_na(flipper_length_mm, bill_length_mm)
scatter_plot(p, p$flipper_length_mm, p$bill_length_mm, p$species, 50, 10, c("flipper length", "in mm"), c("bill length", "in mm"), "Legend") %>% add_title("Relationship between flipper length and bill length","","") %>% SVGrenderer(height=300)
#----
p <- penguins %>%
  drop_na(flipper_length_mm, bill_depth_mm)
scatter_plot(p, p$flipper_length_mm,p$bill_depth_mm, p$species, 50, 5, c("flipper length", "in mm"), c("bill depth", "in mm"), "Legend") %>%add_title("Relationship between flipper length and bill depth","","") %>% SVGrenderer(height=300)
#----
p <- penguins %>%
  drop_na(flipper_length_mm, body_mass_g)
scatter_plot(p, p$flipper_length_mm, p$body_mass_g, p$species, 50, 1000, c("flipper length", "in mm"), c("body mass", "in g"), "Legend") %>% add_title("Relationship between flipper length and body_mass","","")  %>% SVGrenderer(height=300)
#---body mass on the x axis ----
p <- penguins %>%
  drop_na(body_mass_g, bill_length_mm)
scatter_plot(p, p$body_mass_g, p$bill_length_mm, p$species, 1000, 10, c("body mass", "in g"), c("bill length", "in mm"), "Legend") %>% add_title("Relationship between body mass and bill length","","")  %>% SVGrenderer(height=300)
#----
p <- penguins %>%
  drop_na(body_mass_g, bill_depth_mm)
scatter_plot(p, p$body_mass_g, p$bill_depth_mm, p$species, 1000, 5, c("body mass", "in g"), c("bill depths", "in mm"), "Legend") %>% add_title("Relationship between body mass and bill depth","","") %>% SVGrenderer(height=300)
#----
p <- penguins %>%
  drop_na(body_mass_g, flipper_length_mm)
scatter_plot(p, p$body_mass_g, p$flipper_length_mm, p$species, 1000, 50, c("body mass", "in g"), c("flipper length", "in mm"), "Legend") %>%add_title("Relationship between body mass and flipper length","","") %>% SVGrenderer(height=300)
...
colnames(df) <- c("counts")
column_chart(df, breaks, series=c("counts")) %>%
  add_title("Histogram of flipper length", "length","in mm") %>% SVGrenderer(height=300)
#--- body mass histogram ---
...
colnames(df) <- c("counts")
column_chart(df, breaks, series=c("counts")) %>%
  add_title("Histogram of body mass", "mass","in kg") %>% SVGrenderer(height=300)
...
df$Chinstrap <- as.numeric(df$Chinstrap)
df$Gentoo <- as.numeric(df$Gentoo)
column_chart(df, df$Year, c("Adelie", "Chinstrap", "Gentoo")) %>% SVGrenderer(height=300)
